...
app.get('/backend/backendupdateschedule', (req, res) => {
...
app.post('/backend/backendOrdersAdd', (req, res) => {
  ...
  con.query(sql, (err, data) => {
    if (err) {
      ...
      return res.status(200).send({
        resMsg: 'Orders are not allowed to be placed outside of scheduled hours.',
      });
    } else {
                  ...
                  if (err) {
                    ...
                    (async () => {
                      ...
                      for (let x = 0; x < orderItems.length; x++) {
                        ...
                        let [rowsInner, fieldsInner] = await connPromise.execute(
                          sqlInner
                        );
...
app.patch('/backend/backendUpdateMenuData', (req, res) => {
  ...
  let sql = mysql2.format('SELECT * FROM menu_items WHERE menu_items_id=?;', [ID]);
  con.query(sql, (err, data) => {
    if (err) {
      return res.status(200).send({
        resMsg:
...
app.patch('/backend/backendUpdateOrderData', (req, res) => {
  const { ID, locationID, status, note, entry_user, modificationUser } = req.body;
...
app.patch('/backend/backendUpdateUserData', (req, res) => {
  ...
  let sql = mysql2.format('SELECT * FROM users WHERE users_username=?;', [username]);
  ...
  con.query(sql, (err, data) => {
    if (err) {
      ...
      return res.status(200).send({
        resMsg: 'The user that you were trying to update was not able to be found.',
      });
    } else {
      bcrypt.hash(password, saltRounds, (err, hash) => {
        if (err) {
          ...
          con.execute(sql, (err, result) => {
            if (err) {
              return res.status(200).send({
                resMsg: 'An error occurred while updating the user in the system.',
              });
            } else {
              return res.status(200).send({
                resMsg: `${result.affectedRows} row(s) were affected by the update.`,
...
app.patch('/backend/backendUpdateScheduleData', (req, res) => {
  ...
  let sql = mysql2.format('SELECT * FROM order_hours WHERE order_hours_day=?;', [day]);
  con.query(sql, (err, data) => {
    if (err) {
      return res.status(200).send({
        resMsg:
      ...
      return res.status(200).send({
        resMsg: 'The day that you were trying to update has since been removed.',
      });
    } else {
      let sqlTemp =
      ...
      let sql = mysql2.format(sqlTemp, [openHour, closeHour, modificationUser, day]);

      con.execute(sql, (err, result) => {
        if (err) {
          return res.status(200).send({
            resMsg: 'An error occurred while updating the schedule in the database.',
          });
        } else {
          return res.status(200).send({
            resMsg: `${result.affectedRows} row(s) were affected by the update.`,
...
app.get('/backend/backendLoadScheduleData', (req, res) => {
  ...
  con.query(mysql2.format(sqlTemp, []), (err, data, meta) => {
    if (err) {
      return res.status(200).send({ resMsg: 'Error loading schedule from database.' });
    } else if (data.length === 0) {
      return res.status(200).send({
        resMsg:
